 ...
 *
 * Y no arranca solo: espera el clic de quien mira. playsinline=1 evita que en
 * el iPhone el video se apodere de toda la pantalla al empezar.
 */
export function urlDeEmbed(url: string): string {
  ...
  return `${url}${separador}cc_load_policy=0&enablejsapi=1&playsinline=1`;
}
